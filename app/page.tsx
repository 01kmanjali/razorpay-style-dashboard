...
export default function Page() {
  const [activeUser, setActiveUser] = useState(yourRank[2]);
  ...
  return (
      <main className="flex-1">
        ...
        <div className=" lg:col-span-full card p-6 card h-[48px] px-6 py-6 gap-6  rounded-xl flex items-center">
          ...
          <span className="text-sm justify-start text-muted">Your <span className="text-accent-2"> next</span> level is just one <span className="text-accent-2">click away...</span></span>
        </div>
          {/* Earnings Insights */}
          ...
          <section className="card p-6">
            ...
            <div className="text-2xl font-semibold mb-4 text-accent-2">{activeUser && activeUser.rank}</div>
            <div className="space-y-4">
              {yourRank.map((p, index) => (
                <div key={p.name} className="flex items-center justify-between" onClick={() => {setActiveUser(p);}}>
                  <div className="flex items-center gap-3">
                    <div className="h-9 w-9 rounded-full bg-white/15 overflow-hidden">
                      <Image src={p.Image} alt={p.name}  className="h-full"/>
                    </div>
                    <div className="">
                  ...
                  <div className={cn( 
                    (activeUser.name == p.name) ? "text-lg text-accent-2" : "text-sm text-muted")}>{p.rank}</div>
                </div>
              ))}
            </div>
          </section>
